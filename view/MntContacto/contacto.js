...
function editar(con_id){
    ...
    $.ajax({
        url: "../../controller/contacto.php?op=mostrar",
        data: { con_id: con_id },
        type: "POST",
        dataType: "json",
        beforeSend: function() {
...
function eliminar(con_id){
    swal({
        ...
        if (isConfirm) {
            $.ajax({
            url: "../../controller/contacto.php?op=eliminar",
            type: "POST",
            data: { con_id: con_id },
            beforeSend: function() {
...
$(document).ready(function(){
    ...
    $("#car_id").select2({ placeholder:"Seleccione una opción"});
        /* llenar el combo de cargo */
    $.post("../../controller/cargo.php?op=combo",function(data, status){
    $('#car_id').html(data);
    });
    
    $("#cli_id").select2({ placeholder:"Seleccione una opción"});
